	...
	return (
		...
		>
			<div className="bg-slate-900/90 backdrop-blur-xl rounded-2xl p-8 w-full max-w-md shadow-2xl border border-slate-700/50 relative transform transition-all duration-300 hover:shadow-system-primary/10">
				...
				{/* 标题 */}
				<h2 className="text-2xl font-bold bg-gradient-to-r from-white to-system-primary-lighter bg-clip-text text-transparent mb-4">{title}</h2>
				...
				<div className="flex gap-4 justify-end">
					<button
						onClick={onClose}
						className="px-6 py-3 bg-slate-800/80 border border-slate-700/50 rounded-xl font-medium text-white hover:bg-slate-800 hover:border-slate-600/50 transition-all duration-300"
					>
						{cancelText}
					</button>
					<button
						onClick={onConfirm}
						className="px-6 py-3 bg-gradient-to-r from-system-primary to-system-secondary rounded-xl font-medium text-white hover:shadow-lg hover:shadow-system-primary/20 hover:scale-[1.02] transition-all duration-300 active:scale-95"
					>
						{confirmText}
					</button>
				</div>
